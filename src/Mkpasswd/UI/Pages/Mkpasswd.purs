...
type ChildQuery
  = Nav.Query <\/> FormQuery <\/> Const Void
  ...
  eval (Generate next) =
    next
      <$ do
          ...
          let
            policy = (map <<< map) F.unwrapOutputFields $ traverse join policyForm
    ...
    mkpasswdFormless r =
      let
        s = F.unwrapOutputFields r
      in
        mkpasswd s.length s.policy

-- Formless
type FormQuery
  = F.Query Query PolRowQuery PolRowSlot Form Aff
...
validators =
  ...
  enoughLong =
    F.hoistFnE \form ->
      let
        p = fromMaybe [] $ F.getOutput proxy.policy form
      in
        V.chk TooShort $ V.minRule (requiredMinLength p)
...
renderFormless fstate =
  UI.container
    [ HH.div
        ...
        , HH.span
            ...
            [ HH.input
                ...
                , classes
                    [ "input"
                    , if (isJust $ F.getError proxy.length fstate.form) then
                        "is-danger"
                      else
                        ""
                    ]
    ...
    , ( \i ->
          HH.slot DegitsNum F.component i
    ...
    , ( \i ->
          HH.slot UppercaseNum F.component i
            $ HE.input (F.raise <<< H.action <<< OnPolFromMsg)
      )
        { initialInputs:
            F.wrapInputFields
    ...
    , ( \i ->
          HH.slot LowercaseNum F.component i
            $ HE.input (F.raise <<< H.action <<< OnPolFromMsg)
      )
        { initialInputs:
            F.wrapInputFields
    ...
    , ( \i ->
          HH.slot SymbolNum F.component i
            $ HE.input (F.raise <<< H.action <<< OnPolFromMsg)
      )
        { initialInputs:
            F.wrapInputFields
...
-- PolicyForm
type PolRowQuery
  = F.Query Query MulChk.Query Unit PolicyForm Aff

type PolRowSlot
  = FieldType

type PolicyInfo
  = Record (PolicyRow F.OutputType)

newtype PolicyForm r f
  = PolicyForm (r (PolicyRow f))
...
renderPolicyRow fieldType fstate =
  let
    isUsed = F.getInput proxyPolicyRow.isUsed fstate.form
  in
    HH.div
      [ classes [ "field" ] ]
      [ HH.label
      ...
      , HH.div
          ...
          , HH.span
              ...
              [ HH.input
                  ...
                  , classes
                      [ "input"
                      , if (isJust $ F.getError proxyPolicyRow.requiredMinNum fstate.form) then
                          "is-danger"
                        else
                          ""
                      ]
      ...
      , if fieldType == SymbolNum then
          (\i -> HH.slot unit MulChk.ui i (HE.input $ F.setValidate proxyPolicyRow.charSet))
            { allChk: true
